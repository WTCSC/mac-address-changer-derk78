...
check_mac() {
    ...
    #This regular expression works by matching two characters hexdecimal divided by colons repeated five times because a each MAC address have 6 octets, 6 pairs of dexadecimal numbers.
...
#I first run the `check_root` function since I want it to first to make sure the user is running it at the root level.
...
#Prompt the user that the MAC address is being changed.
echo -e "\nChanging the MAC address for the $interface interface to $new_address."

#In order to change the MAC address we need to make sure that we must bring the network interface down first.
